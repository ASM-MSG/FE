...
- **map-home** — `MapHomePage` + ui/: `MapCanvas`(네이버 지도+오버레이), `MapControls`, `CellSummaryPanel`, `CellActionRow`, `HomeCellDetailPanel`, `FeedVideoCard`, `FeedVideoList`(격자 상세·테마 피드 공용), `VideoMiniPanel`(실 재생), `VideoOwnerMeta`, `ThemeChip`/`ThemeChipsBar`/`ThemeFeedPanel`, `naver-sdk-loader.ts`(SDK 격리 경계), `use-escape-close.ts`, `use-home-entry-lifecycle.ts`
- **explore** — `ExplorePanel`(388px 오버레이) + ui/: `ExploreCellCard`, `SortChip`
...
- **cell-detail** — `CellDetailSheet`, `CellMoreMenu`(Radix DropdownMenu), `ReportDialog`, `ReportReasonSelect`
...
- **explore** — model: `explore-cells`, `cell-detail`, `recent-history`, `report`, 스토어 `cell-detail-store`·`explore-filter-store` · ui: `SearchBox`, `RegionRow`, `RecentSearchRow` · api/ 없음
- **map-home** (최대 도메인) — 오버레이/기하: `grid-overlay`, `occupied-grid-overlay`(EPSG:5179), `cluster-overlay`, `theme-overlay`, `grid-label`, `cell-viewport`, `map-scale` · 도메인: `theme`, `theme-feed`, `home-cell-detail`, `grid-videos`, `video-playback`, `viewport-query`, `map-query-policy` · 스토어: `viewport-store`(플랫폼 중립), `theme-filter-store`, `home-cell-detail-store`, `video-mini-panel-store` · 쿼리 훅: `use-cells-query`, `use-occupied-grids-query`, `use-hotzones-query`, `use-grid-detail-query`, `use-grid-videos-query`, `use-video-playback-query` · ui/ 없음(UI는 pages/map-home)
...
- **region** — `regions.ts`, `boundary-geometry.ts`(pointInPolygon·clipLineToBoundary), `busan-boundary.ts`(BUSAN_BBOX)
...
- 어댑터(RN 경계 경유지): `storage.ts`, `navigation.ts`, `geolocation.ts`, `region-lookup.ts` · 유틸: `format.ts`
...
## 테스트 자산 (apps/web/src — 104개, smoke 18개)

레이어별 분포: app 3 · entities 7 · features/auth 6 · features/dex 9 · features/explore 6 · features/map-home 24 · features/profile 7 · features/upload 14 · pages 15 · shared 8 · widgets 3. 목록은 `**/*.test.*`·`**/*.smoke.test.tsx` glob으로 확인.
...
존재 (Expo Router + NativeWind). features 대부분 스텁, `shared/format.parity.test.ts`로 웹/네이티브 동등성 검증. 웹 티켓에서는 통상 무관 — 크로스 플랫폼 티켓만 소스 확인.
