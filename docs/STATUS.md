...
- **dex** — `DexPanel` + ui/: `DexProfileHeader`, `DexStatCards`, `DexTabs`, `RegionProgress`, `RecentRegionRow`(동 단위 행), `BadgeTabBody`(+대표 뱃지 편집모드 — MSG-413), `BadgeMedal`(메달 아트 + 편집모드 선택 표시), `FeaturedProfilePreview`(대표 뱃지 프로필 미리보기 카드), `GalleryTabBody`(격자 그룹+1열), `GalleryVideoCard` · 스모크 공용: `dex-fetch-stub.ts`, `dex-test-harness.tsx`
...
- **dex** — model: `dex-summary`(clampPct·탐험 규모 문구), `recent-regions`(격자→동 묶음), `gallery-groups`(동 영상→격자 그룹), `video-new`(NEW 24h), `badge-showcase`(진열장 정렬), `region-label`(행정동명 축약·수집률 표기), `dex-tab`, 스토어 `gallery-region-store`(동+대표 격자)·`recent-removal-store`, 쿼리 `use-collection-query`(summary·grids·badges)·`use-region-videos-query`·`use-region-stat-query`(by-grid 코드 조인·by-point 수집률), `featured-selection`(대표 뱃지 선택 토글 순수 — 최대 2·해제 승격, RN 재사용 대상, MSG-413) · api: `use-badge-mutations`(`useReplaceFeaturedBadges` — PUT badges/featured + findMyBadges invalidate) · ui/ 없음(UI는 pages/dex)
