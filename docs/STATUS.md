...
- **profile** — `ProfilePanel` + ui/: `ProfileHeader`, `ActivityCard`, `SettingRow`, `SettingToggleRow`(ui-web Switch 행 — "알림 받기", MSG-408) · `NotificationSettingsPanel`(388px 알림 설정 상세 — 로딩/오류 게이트+그룹 2개 토글 5행+각주, MSG-409) + ui/: `NotificationToggleRow`(label↔Switch 배선·행 단위 busy), `PermissionBanner`(수신 불가 시 안내 — CTA는 408 use-push-toggle ON 재사용, 미지원은 CTA 없음)
...
- **dex** — model: `dex-summary`(clampPct·탐험 규모 문구), `recent-regions`(격자→동 묶음), `gallery-groups`(동 영상→격자 그룹), `video-new`(NEW 24h), `badge-showcase`(진열장 정렬), `region-label`(행정동명 축약·수집률 표기), `dex-tab`, 스토어 `gallery-region-store`(동+대표 격자)·`recent-removal-store`, 쿼리 `use-collection-query`(summary·grids·badges·upload-history)·`use-region-videos-query`·`use-region-stat-query`(by-grid 코드 조인·by-point 수집률), `featured-selection`(대표 뱃지 선택 토글 순수 — 최대 2·해제 승격, RN 재사용 대상, MSG-413), `upload-grass`(잔디 파생 순수 — 24/53주 격자·레벨 0/1/2/3/4+·요약 4종·월 경계 라벨, KST epoch 일수 정수 산술, RN 재사용 대상, MSG-414) · api: `use-badge-mutations`(`useReplaceFeaturedBadges` — PUT badges/featured + findMyBadges invalidate) · ui/ 없음(UI는 pages/dex)
