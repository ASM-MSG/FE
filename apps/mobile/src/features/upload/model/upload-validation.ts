/**
 * 영상 업로드 파일 검증 — 웹 `features/upload/model/upload-validation.ts`의 모바일 복제본
 * (MSG-424 포팅, `upload-validation.parity.test.ts`가 동등성을 고정한다).
 * 길이 실측은 웹이 `use-video-duration`(DOM video)으로 하지만 모바일은 expo-image-picker
 * 응답의 duration이 그대로 실측값이라 포팅 대상이 아니다 — 여기는 양쪽 모두 판정만 한다.
 */
