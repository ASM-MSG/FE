/**
 * 업로드 오케스트레이션 — 웹 `features/upload/model/upload-orchestration.ts`의 모바일
 * 복제본 (MSG-424 포팅, `upload-orchestration.parity.test.ts`가 동등성을 고정한다).
 * 이펙트 주입형이라 플랫폼 차이(웹 File/Blob vs RN 파일 uri)는 전부 api 계층
 * (`api/upload-effects.ts`)이 흡수하고 이 상태 머신은 그대로 공유된다.
 */
