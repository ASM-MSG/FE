/**
 * AI 하이라이트 추천 선택/트리머 — 웹 `features/upload/model/highlight-selection.ts`의
 * 모바일 복제본 (MSG-424 포팅, `highlight-selection.parity.test.ts`가 동등성을 고정한다).
 *
 * 환류 F2 — **AI 추천의 선정 사유가 서버 응답에 없다.** `HighlightPreviewResponseDto`가
 * `highlights: number[][]`(시간쌍)만 주므로 추천 행에 사유를 표시하지 않는다(D2).
 * 티켓 요구 4의 "선정 사유"는 이 때문에 의도적 미이행이며, 노출하려면 서버가 구간별
 * 근거(스코어·태그 등)를 함께 내려줘야 한다. Figma의 "움직임·밝기 지속" 등은
 * 플레이스홀더다(오탐 방지 1·2).
 */
 ...
 *
 * 상한을 하한 아래로 내려보내지 않는다: 5초 미만 원본은 select-validation이 일부러
 * 통과시키는 정상 경로인데(길이 하한 없음), 초기 구간이 {0, duration}이라 `끝-5초`가
 * 음수다. min>max로 역전되면 clamp가 그 음수를 그대로 돌려줘 start가 음수로 새어나간다
 * (슬라이더 ratio가 표시만 가리고 상태·서버 트리밍 요청에는 음수 시각이 남는다).
 * 최소 길이를 만족시킬 여지가 없는 구간이므로 시작은 0 고정이 유일한 정의다. (PR #84 리뷰)
 */
