 ...
 * MSG-562: `video` 슬롯이 상태 형상에 더해져 기존 전체 상태 단정에 `video: null`을 추가했다.
 */
...
describe("eventBack — 개요 → 목록 → 해제 → 최상위 (AC 8·D14)", () => {
  ...
  it("목록(방 없음)에서는 이벤트 모드를 해제한다", () => {
    expect(
      eventBack({ active: true, room: null, location: null, video: null }),
    ).toEqual({
      active: false,
      room: null,
      location: null,
  ...
  it("이벤트 모드가 아니면 null — 화면이 자기 규칙으로 넘어간다", () => {
    expect(
      eventBack({ active: false, room: null, location: null, video: null }),
    ).toBeNull();
  });
});
...
describe("withEventRoom — 같은 행사 재탭 무변화, 다른 행사 교체 (D16)", () => {
  ...
  it("다른 행사면 방을 교체하고 이벤트 모드를 켠다", () => {
    expect(
      withEventRoom(
        { active: true, room: ROOM, location: LOCATION, video: null },
        OTHER,
      ),
    ).toEqual({
      active: true,
...
describe("eventBack — 위치 → 개요 → 목록 → 해제 4단 (AC 4·D1)", () => {
  it("위치 상세에서는 위치만 닫고 개요로 돌아간다", () => {
    expect(
      eventBack({ active: true, room: ROOM, location: LOCATION, video: null }),
    ).toEqual({ active: true, room: ROOM, location: null, video: null });
  });
});
...
/**
 * AC 2·11 (MSG-562 D1·D11): `video` 슬롯이 붙어 뒤로가기가 5단(영상 → 위치 → 개요 → 목록 →
 * 해제)이 되고, 위치 선택·다른 행사 open·해제가 영상을 리셋하며 같은 행사 재open은 유지한다.
 * 영상 단도 시트 1단계다.
 */
describe("eventBack — 영상 → 위치 → 개요 → 목록 → 해제 5단 (AC 2·D1)", () => {
  it("영상 상세에서는 영상만 닫고 위치 상세로 돌아간다", () => {
    expect(
      eventBack({
        active: true,
        room: ROOM,
    ...
    ).toEqual({ active: true, room: ROOM, location: LOCATION, video: null });
  });

  it("영상 단에서도 시트는 1단계다 (AC 2)", () => {
    expect(
      eventSheetStage({
        active: true,
        room: ROOM,
  ...
  it("다른 행사면 영상까지 리셋하고, 같은 행사면 상태 객체가 그대로다 (AC 2)", () => {
    const state = {
      active: true,
      room: ROOM,
      location: LOCATION,
    ...
    expect(withEventRoom(state, { ...ROOM })).toBe(state);
    expect(withEventRoom(state, OTHER)).toEqual({
      active: true,
      room: OTHER,
      location: null,
      video: null,
...
describe("영상 선택 모듈 상태 (AC 2·11·D1·D11)", () => {
  beforeEach(() => {
    deactivateEvent();
  });

  it("selectEventVideo가 영상 id를 세우고 구독자에게 알린다 (AC 2)", () => {
    const listener = vi.fn();
    const unsubscribe = subscribeEventSelection(listener);
    activateEvent();
    openEventRoom(ROOM);
  ...
  it("다른 위치 선택은 영상을 리셋한다 — 플레이어가 언마운트된다 (AC 11)", () => {
    activateEvent();
    openEventRoom(ROOM);
    selectEventLocation(LOCATION);
    selectEventVideo(VIDEO_ID);
    ...
    expect(getEventSelection().video).toBeNull();
    expect(getEventSelection().location?.locationId).toBe(10);
  });

  it("같은 행사 재open은 영상을 유지하고 다른 행사 open은 리셋한다 (AC 2)", () => {
    activateEvent();
    openEventRoom(ROOM);
    selectEventLocation(LOCATION);
    selectEventVideo(VIDEO_ID);
    ...
    openEventRoom({ ...ROOM });
    expect(getEventSelection().video).toBe(VIDEO_ID);

    openEventRoom(OTHER);
    expect(getEventSelection().video).toBeNull();
  });

  it("✕(deactivate)는 영상까지 전부 비운다 (AC 11)", () => {
    activateEvent();
    openEventRoom(ROOM);
    selectEventLocation(LOCATION);
    selectEventVideo(VIDEO_ID);
  ...
  it("‹·하드웨어 백 5단 — 영상 → 위치 → 개요 → 목록 → 해제 후 false (AC 2)", () => {
    activateEvent();
    openEventRoom(ROOM);
    selectEventLocation(LOCATION);
    selectEventVideo(VIDEO_ID);
