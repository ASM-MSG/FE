 ...
 * 2단계 확장점: `location` 슬롯 + `eventBack` 3단 (웹 store `:83-93`).
 *
...
const INACTIVE: EventSelectionState = { active: false, room: null };

/** 뒤로가기 한 단계 — 개요 → 목록 → 해제. 이벤트 모드가 아니면 null(화면 규칙으로) [D14] */
export const eventBack = (
  state: EventSelectionState,
): EventSelectionState | null => {
  if (state.room !== null) return { active: true, room: null };
  if (state.active) return INACTIVE;
  return null;
};

/** 시트 스냅 — 목록 2단계(절반), 개요 1단계(전체) [D15] */
export const eventSheetStage = (state: EventSelectionState): SheetStage =>
  state.room !== null ? 1 : 2;

/** 카드 탭 — 같은 행사 재탭은 무변화(같은 참조), 다른 행사는 교체 [D16] */
