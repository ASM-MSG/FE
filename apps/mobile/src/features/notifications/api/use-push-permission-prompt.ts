 ...
 * 한 번 돈다.
 *
 * 여기가 물을 자리인 이유: 업로드를 막 끝낸 순간이 알림의 맥락이 성립하는 자리다(MSG-567에서
 * 블러 안내 문장은 사라졌지만 프롬프트 시점·흐름은 불변 — A1). MSG-429는 등록 진입점을 프로필
 * 토글로만 두었기 때문에, 프로필 설정에 들어가 본 적 없는 사용자에게는 알림이 갈 확률이 0이었다.
 *
