...
/**
 * 업로드 잔디 파생 (MSG-430 L4·L5) — 웹 `features/dex/model/upload-grass.ts` 미러
 * (parity 테스트가 고정). GitHub 기여그래프 구조: 주 시작=일요일(행 0),
 * 오늘이 속한 주가 마지막 열. 기록 탭은 24주, 1년 화면은 53주 창을 같은 함수로 파생한다.
 *
 * KST 안전 날짜 산술 [L5]: uploadDate는 KST 날짜 라벨("YYYY-MM-DD")이다.
 * `new Date("YYYY-MM-DD")`는 UTC 자정으로 파싱돼 기기 TZ에 따라 하루가 밀리므로,
...
/** 1년 화면 잔디 창 — 53주 (GitHub 연간 그래프 구조) */
...
/** 한 주(열) — index 0=일요일. 마지막 주의 미래 날짜는 null(자리 없음) */
export type GrassWeek = (GrassCell | null)[];

/** 잔디 격자 파생 — weekCount열×7행, 희소 이력의 빈 날은 0으로 채운다 [L4] */
...
/**
 * 월 라벨 파생 [L4] — 각 열의 첫 날(일요일)이 이전 열과 다른 달에 진입하는 열 위에
 * 라벨을 단다(첫 열 포함, GitHub 관례). 시안의 4주 간격 균등 배치는 플레이스홀더 —
 * 실제 월 경계 기준이 정본이다 (스펙 "Figma 오탐 방지").
 */
...
export interface GrassSummary {
  ...
  /** 창 내 최장 연속 업로드 일수 — 상단 스트릭 타일(currentStreak: 현재 진행 중)과 다른 지표다 */
  longestStreak: number;
  /** 창 내 하루 최다 업로드 개수 */
  maxDailyCount: number;
  /**
   * 가장 활발한 요일("토요일") — 업로드 '일수' 기준. 동률이면 주 시작(일요일)부터
   * 앞선 요일. 창 내 업로드가 없으면 null(1년 화면 부제에서 해당 조각 생략)
   */
  mostActiveWeekday: string | null;
}

/** 요약 파생 [L4] — 탭 메타(24주)·1년 헤드라인/부제(53주)가 같은 함수를 창만 달리해 쓴다 */
