...
/**
 * 프로필 이미지 순수 로직 (MSG-426 — 웹 `features/profile/model/profile-image.ts` 포팅,
 * parity 테스트가 고정한다). 상수·검증·파라미터 도출·에러 문구·캐시 병합.
 * 플랫폼 API(File·DOM 등) 무참조 — 중립 형태만 받는다.
 *
 * 이번 티켓이 소비하는 것은 `profileImageErrorMessage`(기본 이미지 되돌리기 실패 안내)뿐이지만,
 * 티켓 [참고]가 profile-image 전체 포팅을 지시해 7종을 모두 옮겼다 — 업로드 전용 심볼은
 * 후속 이미지 업로드 티켓의 진입점이다 (스펙 Q3 결정).
 */

/**
 * 파일 선택 accept — jpeg·png·webp MIME 타입만.
...
/** 최대 프로필 이미지 크기 — 5MB(바이트). 경계값 5MB 정확히는 허용 */
export const MAX_PROFILE_IMAGE_BYTES = 5 * 1024 * 1024;

/** 크기가 5MB 이하인지 판정한다 — 경계 5MB 포함 */
...
/**
 * 중립 후보에서 presign 요청 파라미터를 도출한다.
 * 확장자는 소문자 정규화, 확장자가 없거나 빈 이름("photo"·"photo.")은 무효(null).
 */
...
/**
 * 클라이언트 형식 무효 판정 에러 — 확장자 없는 파일 등 presign 요청 없이 거른 경우.
 * profileImageErrorMessage가 서버 1415와 같은 형식 안내 문구로 매핑한다.
 */
...
/** 5MB 초과 사전 검증 안내 문구 — 서버 1413과 같은 문구를 화면 사전 검증이 재사용 */
export const PROFILE_IMAGE_SIZE_MESSAGE = SIZE_MESSAGE;

/**
 * 이미지 실패 에러를 사용자 안내 문구로 매핑한다.
...
/**
 * 확정 응답을 기존 캐시 데이터(getMe 봉투의 data)에 병합한다 — profileImageUrl만 반영.
 * 이 뮤테이션이 바꾸는 값은 이미지뿐이다 — 나머지 필드(email·nickname·createdAt)는
 * 조회(getMe)가 정본이라 여기서 덮지 않는다.
 */
