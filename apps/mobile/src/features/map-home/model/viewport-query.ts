...
/**
 * 뷰포트 → `GET /api/grids` 요청 판정·변환 — 웹 `features/map-home/model/viewport-query.ts`의
 * 복제본 (MSG-423 포팅). 순수 함수 — 지도 SDK/플랫폼에 의존하지 않는다.
 * 동등성은 viewport-query.parity.test.ts가 웹 원본을 동적 import해 단정한다.
 */
...
/**
 * 뷰포트 bbox를 받는 쿼리의 공통 인자 — 요청 파라미터와 실행 여부를 한 번에 만든다.
 * 미요청 상태의 query 값은 enabled가 false라 실제로 나가지 않지만, 훅 옵션 타입이
 * 요구해 0으로 채운다.
 */
