...
import { pointInPolygon } from "../../../entities/region/model/boundary-geometry";
import { parseLineString } from "./course";
import type { MissionResponseDto } from "../../../shared/api/sdk";
import type { ThemeId } from "./themes";

/**
 * 활성 미션(`GET /api/missions/active`) 분류·기하 파생 (MSG-427 D1·D9·F-12) —
 * 웹 `features/map-home/model/mission.ts`의 복제본.
 * 순수 함수 — 지도 SDK/플랫폼에 의존하지 않는다.
 * 동등성은 mission.parity.test.ts가 웹 원본을 동적 import해 단정한다.
 *
 * 서버는 축제·팝업·코스를 유형별로 주고 렌더 shape가 판별자 없는 유니온이라
 * **런타임 판별이 필요하다**. 판별을 `type`이 아니라 shape 필드 유무로 하는 이유:
 * 명세가 POPUP의 shape를 규정하지 않아, type 표에 기대면 팝업이 통째로 빈 영역이 된다.
 *
 * **격자를 미리 펼치지 않는다** (승인 Q5의 전제): BOX 미션 하나가 900m 사방(9×9=81칸)이라
 * 전 미션의 격자를 나열하면 좌표 변환(proj4)이 수만 번 돌고 지도가 멈춘다(웹 렉 사고 이력).
 * 지도 타일은 **뷰포트에 걸치는 미션만, 뷰포트 범위 안에서만** 펼친다 (F-12).
 */

/** 미션이 실리는 칩 — 상단 칩 4종 중 핫구역을 뺀 3종 */
export type MissionChip = Extract<ThemeId, "festival" | "popup" | "route">;

/** 활성 테마 칩 → 미션 칩. 핫구역에는 미션이 없다. [D1] */
export const missionChipOfTheme = (
  theme: ThemeId | null,
): MissionChip | null => (theme === null || theme === "hot" ? null : theme);

/** 미션 칩 → `GET /api/missions/active`의 `type` 파라미터. [D1] */
...
/**
 * 이 격자가 미션 영역 안인가.
 * 미션 격자를 펼쳐 교집합하는 대신 이 판정을 격자마다 부른다 — BOX 미션에서는 좌표 변환을
 * 아예 하지 않게 된다.
 */
...
/**
 * 뷰포트 안에 들어오는 미션 격자 id. [F-12]
 * 화면 밖 격자는 어차피 안 보이므로 펼치지 않는다 — 전 영역을 펼치던 것이 웹에서
