/**
 * 미션 상태 배지 파생 (MSG-427 D5) — 웹 `features/map-home/model/mission-status.ts`의 복제본.
 * 순수 함수 — 지도 SDK/플랫폼에 의존하지 않는다.
 * 동등성은 mission-status.parity.test.ts가 웹 원본을 동적 import해 단정한다.
 *
 * `now`를 주입받는다 — "오늘"이 판정에 들어가므로 호출부가 시각을 소유해야 테스트가
 * 시계에 흔들리지 않는다. 명세의 `startAt`/`endAt`은 타임존 없는 로컬 표기라 달력 일자
 * 비교도 로컬 기준으로 맞춘다.
 */
