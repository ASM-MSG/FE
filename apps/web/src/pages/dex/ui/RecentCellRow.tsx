...
interface RecentCellRowProps {
  cell: CollectedCell;
  /**
   * 행 클릭 — 행의 셀 전체를 전달하고 배선(지도 이동 + 그 지역 갤러리 전환)은 부모 몫
   * (AC 16, MSG-122 AC 19 개정 — district가 필요해 center만이 아닌 셀을 올려보낸다)
   */
  onSelect: (cell: CollectedCell) => void;
 ...
 * 행 클릭은 지도 이동 + 그 지역 갤러리 전환이다(MSG-122 AC 19 개정 — 배선은 DexPanel).
 * 선택 버튼과 X 버튼은 형제 요소다(중첩 버튼 금지) — X 클릭은 stopPropagation으로
 * 행 클릭 전파를 차단하고, 행별 접근 가능한 이름을 가진다 (AC 25).
