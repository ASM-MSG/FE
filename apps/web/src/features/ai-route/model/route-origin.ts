...
import type { OriginDto } from "@/shared/api/generated";

/**
 * 출발지 자동 판정 (MSG-489 L1~L5).
 * 순수 함수 — 지도 SDK도 `navigator`도 모른다(RN 재사용 대상). 현위치 조회는
 * `shared/geolocation` 어댑터가, 뷰포트는 `viewport-store`가 공급한다.
 *
 * 토글이 아니다: 현위치가 지금 보이는 지도 범위 안에 있을 때만 출발지를 싣고,
 * 밖이거나 권한 거부·미확보면 조용히 생략한다(기능은 그대로 동작한다 — D8).
 */

/** 뷰포트 포함 판정 — 경계선 위는 안으로 본다 (L5). 2차 발사의 새 bounds 확인도 이 판정을 쓴다 */
