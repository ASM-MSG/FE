...
const progressDto = (
  ...
  targetCount: number,
  completed: boolean,
) => ({ missionId, filledCount, targetCount, completed });

describe("toMissionView — 카드·상세·오버레이가 공유할 표시 모델 (AC 13·20)", () => {
  it("격자·서버 진행도·상태를 한 번에 붙인다 (AC 13·20)", () => {
  ...
  it("진행도를 아직 못 받았으면 기간 배지가 그대로 보인다 (AC 4·20)", () => {
    const view = toMissionView(
...
describe("toCourseView — 코스 전용 파생을 얹는다 (AC 21~22)", () => {
  const courseDto = dto({
    missionId: 3,
    type: "COURSE",
    title: "남파랑길 3코스",
  ...
  it("라인과 코스 순서 스팟을 함께 만든다 (AC 21)", () => {
    const view = toCourseView(
      courseDto,
      progressDto(3, 1, 3, false),
      new Set(["s1"]),
      NOW,
    );
  ...
  it("스팟 방문 여부는 서버 스팟 통계를 따른다 (AC 21)", () => {
    const view = toCourseView(
      courseDto,
      progressDto(3, 1, 3, false),
      new Set(["s1"]),
    ...
    expect(view.spots[1].visited).toBe(false);
  });

  it("진행도는 서버가 준 방문 수·목표 수다 (AC 20)", () => {
    const view = toCourseView(
      courseDto,
      progressDto(3, 1, 3, false),
      new Set(["s1"]),
  ...
  it("코스가 아닌 미션을 넘기면 라인·스팟이 비어 있다 (경계)", () => {
    const view = toCourseView(dto({}), undefined, new Set(), NOW);
