...
/**
 * 선택 정보(cover·stat) 일괄 대기 유예 상한 — 정상 응답(수백 ms)은 전부 한 번에 그려지고,
...
export const useGridDetailQuery = (
  ...
  // 일괄 렌더 게이트 — 선택 정보(cover·stat)도 정착(성공/실패)할 때까지 로딩을 유지한다.
  // 세 쿼리는 병렬이라 총 소요는 그대로인데, 부분 렌더는 작은 카드에서 레이아웃이
  // 연쇄로 밀리는 점진 등장만 만든다 — 마지막 응답 시점에 한 번에 그린다 (실패는 빈 분기).
  // 단 유예(OPTIONAL_SETTLE_GRACE_MS)까지만 — 경과하면 격자 정보만으로 그린다
  if ((cover.isPending || stat.isPending) && !graceOver) {
