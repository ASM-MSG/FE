...
describe("cellToBounds — 격자 중심+한 변 → 사각 Bounds 변환 (AC 10)", () => {
  ...
  it("반환된 bounds의 중점이 입력 중심과 일치한다", () => {
    const { sw, ne } = cellToBounds(SEOUL, 500);

    expect((sw.lat + ne.lat) / 2).toBeCloseTo(SEOUL.lat, 10);
    expect((sw.lng + ne.lng) / 2).toBeCloseTo(SEOUL.lng, 10);
  });
  ...
  it("경도 스팬은 위도별 보정된다 — 서울 위도에서는 위도 스팬을 cos(lat)로 나눈 값", () => {
    const { sw, ne } = cellToBounds(SEOUL, 500);
    const latSpan = ne.lat - sw.lat;
    const expectedLngSpan = latSpan / Math.cos((SEOUL.lat * Math.PI) / 180);
  ...
  it("한 변 0이면 중심 한 점으로 퇴화한다 (sw === ne === center)", () => {
    const { sw, ne } = cellToBounds(SEOUL, 0);

    expect(sw).toEqual(SEOUL);
    expect(ne).toEqual(SEOUL);
  });
