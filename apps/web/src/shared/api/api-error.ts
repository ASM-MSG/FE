/**
 * API 에러 정규화 (MSG-323 수용 기준 7·8).
 *
 * HTTP 에러 봉투·비봉투 바디·네트워크 실패를 단일 `ApiError`로 통일해
 * 화면 코드가 봉투·HTTP 계층을 모르게 한다. `normalizeApiError`는
 * hey-api 클라이언트의 `interceptors.error`에서 호출된다 (error-interceptor.ts).
 */

/**
 * 정규화된 API 에러 — TanStack Query의 error 기본 타이핑(Error)과 정합하고
 * `instanceof ApiError`로 화면 코드가 안전하게 분기한다 (스펙 질문 1 승인).
 *
 ...
 *
 * - 7a. 봉투 바디(예: 401 `{developCode: 2403, ...}`) → status·developCode·message
 * - 7b. 봉투가 아닌 바디(문자열·파싱 불가 JSON·빈 바디) → status·message만
 * - 7c. 네트워크 실패(fetch reject — response 없음) → status 없는 ApiError
 */
...
/** 재시도 최대 횟수 — 최초 시도 제외 (총 최대 3회 시도, 스펙 질문 9 승인) */
