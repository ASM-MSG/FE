...
2. 구현 착수 전 테스트를 실행해 RED를 직접 확인한다 — 실패를 보지 않은 테스트는 무엇을 검증하는지 알 수 없다. 기능·모듈 부재가 이유인 실패만 정당한 RED다(신규 모듈의 import 실패 포함), 오타·셋업 오류는 고치고 재실행한다. 즉시 통과하면 둘 중 하나로 판정한다: 스펙이 기존 동작 유지를 요구하는 보존 단정이면 정당 — 빌드 리포트에 "red 대상 아님(보존 단정)"으로 명시한다. 신규 동작인데 통과면 아무것도 증명하지 못하므로 테스트를 재설계한다
