...
1. 스펙의 로직형 수용 기준을 실패하는 vitest 테스트로 옮긴다. **작성 전에 이 스킬의 `references/test-templates.md`를 읽고 선택 매트릭스에서 기준 유형에 맞는 템플릿(①순수 로직/②스토어/③쿼리 훅/④컴포넌트 스모크/⑤흐름 스모크)을 골라 그 구조와 단정 범위를 따른다** — 매트릭스 밖 형태(스냅샷·testid 쿼리·단정 없는 실행·구현 세부 단정)는 만들지 않는다. 테스트 이름은 수용 기준 문장을 반영한다 — 테스트가 곧 기획의 번역이다
...
1. **인벤토리 먼저**: `docs/STATUS.md`(구현 현황 정본)의 ui-web 인벤토리·해당 도메인 절을 읽고 시작한다. `packages/ui-web/src/index.ts` 직접 확인은 STATUS.md와 어긋나 보이거나 컴포넌트 시그니처가 필요할 때만 — 어긋나면 코드가 정본이고 빌드 리포트에 기록한다
...
- 변경 패키지 스코프 게이트 통과 (3단계 — 루트 풀 게이트는 검증 단계가 최종 1회 실행)
- `_workspace/MSG-{번호}/02_build_report.md` 작성: 변경 파일 목록, 기준별 테스트 매핑(RED 확인 열 포함 — 허용값: 최초 실패(사유 한 줄) 또는 RED 대상 아님(보존 단정)), 스펙 이슈, 결정 기록 여부, **검토한 대안** — 트레이드오프 판단마다 기각한 대안과 기각 이유를 한 줄씩(없으면 "없음"). 머지 후 회고(task-retro)가 "왜 B가 아니라 A였나"를 복원하는 유일한 재료다 — 결정 시점에 안 남기면 소실된다
