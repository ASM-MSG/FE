 ...
 *
 * **본문은 5종 모두 초안이 채워져 있다**(`terms-bodies.ts`, MSG-604 — 법률 검토 전, 운영 주체 명칭 미확정).
 * `body: string | null`의 `null`은 **새 문서를 추가했는데 문구가 아직 없을 때**만 쓴다 — 빈 문자열이나
 * "준비 중입니다" 같은 **가짜 본문을 넣지 않는다**(그러면 "문구가 있다"와 "없다"가 타입으로 구분되지
 * 않는다). 뷰어(앱·웹)는 `isTermsPlaceholder`로 그 상태를 따로 그린다. 조회 API는 아직 없고
 * (생성 SDK에 `term`·`policy` 경로 0건), 생기면 이 카탈로그의 `body`를 조회 결과로 갈아끼우는 것이
 * 유일한 변경점이다(화면은 resolve 결과만 읽는다).
 */
