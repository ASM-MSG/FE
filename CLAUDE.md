...
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-09-24 | 모바일 런북 **7절 iOS 앱스토어 아카이브·업로드 CLI 절차** + **함정 13(pnpm NODE_PATH — Release JS 번들이 Babel 전이 의존성을 못 찾음)** 신설, `apps/mobile/plugins/with-pnpm-node-path.js` config plugin 추가 | docs/MOBILE_RUNBOOK.md, apps/mobile/plugins | MSG-606 첫 앱스토어 제출 — `xcodebuild archive`가 "Bundle React Native code and images"에서 `Cannot find module 'babel-preset-expo'` → 고친 뒤 `'@babel/plugin-transform-react-jsx'`로 연속 실패. Xcode 밖 `expo export:embed`는 성공해 환경 차이로 좁혔고, pnpm `.bin` shim이 주입하는 `NODE_PATH`(`.pnpm/node_modules` 호이스트 디렉터리)가 Xcode 스크립트에는 없는 것이 원인(`node <cli> export:embed`로 shim 없이 재현). `.xcode.env`는 prebuild마다 재생성되므로 config plugin으로 굳혔다. dev client만 돌리던 런북에 릴리스 경로가 없어 처음 밟았다 |
